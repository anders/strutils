...
var (
	flagGenerate = flag.Bool("generate", false, "generate files")
)
...
func TestGenerateBlocks(t *testing.T) {
	if !*flagGenerate {
		t.Skip("won't generate unless -generate is true")
	}

	buf := &bytes.Buffer{}
	buf.WriteString("// generated by gendata_test.go\n")
	buf.WriteString("package strutils\n")

	f, err := os.Open("Blocks.txt")
	if err != nil {
		t.Fatalf("error opening Blocks.txt: %w", err)
	}
	defer f.Close()
